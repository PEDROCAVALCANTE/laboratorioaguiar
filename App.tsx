...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex font-sans text-slate-900">
      ...
      {/* Main Content Area */}
      {/* Reestruturação: overflow movido para cá para permitir banner fixo (sticky) */}
      <main className={`flex-1 transition-all duration-300 overflow-y-auto h-screen relative md:ml-64 bg-slate-50`}>
        
        {/* === NOVO BANNER MINIMALISTA (DESKTOP) === */}
        <header className="hidden md:flex items-center justify-between px-8 py-3 bg-white border-b border-slate-200 sticky top-0 z-30 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
            <div className="flex items-center gap-4">
                {/* Logo Section Minimal */}
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-blue-50 border border-blue-100 flex items-center justify-center p-1.5">
                        <img src="https://iili.io/fYtBdqF.png" alt="Logo" className="w-full h-full object-contain" />
                    </div>
                    <div>
                        <h1 className="text-sm font-bold text-slate-800 leading-tight">Aguiar Prótese Dentária</h1>
                        <div className="flex items-center gap-1.5">
                           <ShieldCheck size={10} className="text-teal-500" />
                           <p className="text-[10px] text-slate-400 font-medium leading-tight">Sistema Administrativo Profissional</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center gap-6">
                ...
                <div className="w-8 h-8 rounded-full bg-slate-800 text-white flex items-center justify-center text-xs font-bold border-2 border-slate-100 shadow-sm">
                   LA
                </div>
            </div>
        </header>
        ...
        {/* Wrapper do Conteúdo */}
        <div className="p-4 pt-20 md:p-8 md:pt-6 max-w-7xl mx-auto space-y-6">
          
          {isLoading && (
            <div className="fixed top-20 md:top-20 right-4 md:right-8 z-50 bg-white/90 backdrop-blur px-4 py-2 rounded-full text-xs font-bold text-teal-600 shadow-md border border-teal-100 flex items-center gap-2 animate-pulse">
